...
macOS builds remain fully functional when no signing credentials are configured. Local packaging produces unsigned DMG and PKG artifacts. The GitHub release workflow runs only for version tags and signs the build only when the protected certificate secrets are available.
...
The workflow creates a temporary keychain on the macOS runner, imports the signing material, signs the application and installer, and deletes the keychain and certificate file after packaging. Protect release tags and restrict access to these secrets to trusted maintainers.
